...
var NodeSol = Class.extend({
    init: function(options) {
        ...
        // unless options.shouldAddPipelineToMessage is False, try to init Uber pipeline
        if (!('shouldAddPipelineToMessage' in options) || options.shouldAddPipelineToMessage) {
            self.pipeline = process.env.UBER_PIPELINE;
            if (!self.pipeline) {
                var pipelineFile = options.pipelineFile || '/etc/uber/pipeline';
                ...
                try {
                    var data = fs.readFileSync(pipelineFile, 'utf8');
                    self.pipeline = data.trim();
                } catch (e) {
    ...
    log_line: function(topic, message, callback) {
        ...
        if (self.pipeline) {
            wholeMessage.pipeline = self.pipeline;
        }
